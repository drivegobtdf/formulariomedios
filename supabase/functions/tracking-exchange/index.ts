...
export default async function handler(req: Request): Promise<Response> {
  ...
  // Contractual rule: POST only to prevent crawlers / pre-fetching from consuming single-use exchange tokens
  if (req.method !== 'POST') {
    return new Response(
      JSON.stringify({ error: 'METHOD_NOT_ALLOWED', message: 'Método no permitido. El canje de seguimiento requiere POST' }),
  ...
  try {
    ...
    const exchangeToken = (body.exchange_token || body.token || '').trim();

    if (!exchangeToken) {
      return new Response(
        JSON.stringify({ error: 'VALIDATION_ERROR', message: 'Token de canje temporal obligatorio' }),
    ...
    const { data, error } = await supabase.rpc('tracking_exchange_core', {
