-- ==============================================================================
-- MIGRATION 015: Harden F4 Functions Security (SECURITY INVOKER & Strict Grants)
-- Proyecto: PEDIDOS — Secretaría de Medios (Revisión 3.0)
-- ==============================================================================

-- 0. Conceder USAGE en esquema private a service_role
GRANT USAGE ON SCHEMA private TO service_role;

-- 1. Redefinir private.ped_calendar_year como SECURITY INVOKER
...
REVOKE ALL ON FUNCTION private.ped_calendar_year(timestamptz) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION private.ped_calendar_year(timestamptz) TO service_role;

-- 2. Redefinir private.reserve_ped_numbers como SECURITY INVOKER
...
REVOKE ALL ON FUNCTION private.reserve_ped_numbers(integer, timestamptz) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION private.reserve_ped_numbers(integer, timestamptz) TO service_role;

-- 3. Redefinir private.compute_submission_fingerprint como SECURITY INVOKER
...
REVOKE ALL ON FUNCTION private.compute_submission_fingerprint(jsonb) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION private.compute_submission_fingerprint(jsonb) TO service_role;

-- 4. Redefinir public.submission_create_core como SECURITY INVOKER
