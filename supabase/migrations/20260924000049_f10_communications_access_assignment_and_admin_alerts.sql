...
-- 3. Helper de Encolado: Alerta a Administradores Activos de Nueva Solicitud Recibida
...
BEGIN
    ...
    -- Obtener lista de pedidos asociados
    SELECT jsonb_agg(
        jsonb_build_object(
            'id', p.id,
    ...
    -- Iterar por cada administrador activo y aprobado
    FOR v_admin IN 
        ...
        WHERE ua.estado_acceso = 'aprobado' 
          ...
          AND trim(au.email) <> ''
    LOOP
        v_idempotency_key := 'pedido_nuevo_admin:' || p_envio_id::text || ':' || v_admin.user_id::text;
        v_comm_id := gen_random_uuid();
